...
public partial class LoginDept : System.Web.UI.Page
{
    ...
    protected void BindDeparment()
    {
        try
        {
            DataTable dt = DB.BindDepartmentAll();

            if (dt != null && dt.Rows.Count > 0)
            {
                ddldeparment.DataSource = dt;
                ddldeparment.DataTextField = "Name";
                ddldeparment.DataValueField = "Id";
                ddldeparment.DataBind();
            }
            else
            {
                ddldeparment.DataSource = null;
                ddldeparment.DataBind();
            }
        }
        catch (Exception ex)
    ...
    protected void btnLogin_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DataTable dt = DB.loginUserdept(ddldeparment.SelectedValue.ToString(), txtPassword.Text.Trim(), ip);
            ...
            String DeptName = ddldeparment.SelectedItem.Text.ToString();// dt.Rows[0]["DeptName"].ToString();
            ...
            if (usertype == "Department")
            {
                ...
                FormsAuthentication.SetAuthCookie(DeptName, false);
                FormsAuthenticationTicket EntReg = new FormsAuthenticationTicket(1, DeptName, DateTime.Now, DateTime.Now.AddMinutes(30), false, DeptName, FormsAuthentication.FormsCookiePath);
                ...
                Response.Redirect("Department/DepartmentView.aspx", false);
