...
public partial class Login : System.Web.UI.Page
{
    ...
    protected void btnLogin_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DataTable dt = DB.loginUser(txtUsername.Text.Trim(), txtPassword.Text.Trim(), ip);
            if (dt.Rows.Count == 0)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "alert", "alert('Please Enter Valid Username or Password!');", true);
                return;
            }

            string UserName = dt.Rows[0]["UserName"].ToString();
            string password = dt.Rows[0]["Password"].ToString();
            if (dt != null && UserName.ToString() != "" && password.ToString() != "")
            {
                Session["RoleID"] = dt.Rows[0]["RoleID"].ToString();
                //Session["CreateProject"] = dt.Rows[0]["CreateProject"].ToString();
                //Session["AbleToReply"] = dt.Rows[0]["AbleToReply"].ToString();
                ...
                //string roleid1 = dt.Rows[1]["RoleID"].ToString();

                if (usertype == "Administrator" && Session["type"].ToString() == "Administrator")
                {
                    RegenerateId();
            ...
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "alert", "alert('Enter Username or Password!');", true);
                return;
            }
        }
        catch (Exception ex)
